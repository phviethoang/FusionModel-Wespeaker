...
trials_dir=${data}/voxceleb1_test/trials
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  if [ ! -e ${trials_dir}/${calibration_trial} ]; then
    python tools/generate_calibration_trial.py --utt2dur ${data}/voxceleb1/utt2dur --trial_path ${trials_dir}/${calibration_trial}
  fi

  python wespeaker/bin/score.py \
    --exp_dir ${exp_dir} \
    --eval_scp_path ${exp_dir}/embeddings/voxceleb1/xvector.scp \
    --cal_mean True \
    --cal_mean_dir ${exp_dir}/embeddings/voxceleb1 \
  ...
  python wespeaker/bin/score_norm.py \
    ...
    --eval_emb_scp ${exp_dir}/embeddings/voxceleb1/xvector.scp \
    --mean_vec_path ${exp_dir}/embeddings/voxceleb1/mean_vec.npy
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  cat ${data}/voxceleb1_test/utt2dur ${data}/voxceleb1/utt2dur > ${exp_dir}/scores/calibration/utt2dur
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  for x in ${trials}; do
    ...
    python wespeaker/bin/compute_metrics.py \
      ...
      2>&1 | tee -a ${scores_dir}/voxceleb1_test_cali_${score_norm_method}${top_n}_result
