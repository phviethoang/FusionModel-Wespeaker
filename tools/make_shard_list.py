    ...
    with tarfile.open(tar_file, "w") as tar:
        for item in data_list:
            try:
                ...
                ts = time.time()
                if wav.endswith('|'):
                    p = subprocess.Popen(wav[:-1], shell=True, stdout=subprocess.PIPE)
                    data = p.stdout.read()
                else:
                ...
                logging.error(f"❌ ERROR while processing {key}: {e}")

    logging.info(f'📊 Finished {tar_file}: read {read_time:.2f}s, write {write_time:.2f}s')
