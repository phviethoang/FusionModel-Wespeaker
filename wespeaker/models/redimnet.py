...
class MultiHeadAttention(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: torch.Tensor, bsz:int):
        if isinstance(seq_len, int):
            seq_len = torch.tensor(seq_len, dtype=torch.long, device=tensor.device)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        tgt_len = torch.tensor(tgt_len, dtype=torch.long, device=hidden_states.device)
        # get query proj
...
class TimeContextBlock1d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Đảm bảo x là Tensor trước khi tiếp tục
        if x is None:
            raise ValueError("Input 'x' cannot be None in forward pass")

        # Gọi red_dim_conv
        x = self.red_dim_conv(x)

        # Kiểm tra xem red_dim_conv có trả về None hay không
        if x is None:
            raise ValueError("Output of red_dim_conv cannot be None")

        # Không cần torch.as_tensor() nếu x đã là Tensor
        # (PyTorch modules như Conv thường trả về Tensor)
        # Nếu bạn không chắc x có phải là Tensor hay không, có thể kiểm tra:
        ...
        # Đảm bảo skip và x có cùng kích thước trước khi cộng
        if skip.shape != x.shape:
            raise ValueError(f"Shape mismatch: skip {skip.shape} and x {x.shape}")
