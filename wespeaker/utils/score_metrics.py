...
def compute_pmiss_pfa_rbst(scores, labels, weights=None):
    ...
    #Debug
    sum_imp_wghts = np.sum(imp_wghts)
    if sum_imp_wghts == 0:
        fpr = np.zeros_like(imp_wghts)
    else:
        fpr = 1 - np.cumsum(imp_wghts) / sum_imp_wghts
...
def compute_eer(fnr, fpr, scores=None):
    ...
    print("FPR:", fpr)
    print("Scores:", scores)
    print("diff_pm_fa:", np.flatnonzero(diff_pm_fa >= 0))

    
    # x1 = np.flatnonzero(diff_pm_fa >= 0)[0]
    #Debug
    valid_indices = np.flatnonzero(diff_pm_fa >= 0)
    if len(valid_indices) == 0:
        print("Warning: No valid x1 found in compute_eer. Setting default EER value.")
        return 1.0, None  # Giá trị mặc định
    x1 = valid_indices[0]

    # x2 = np.flatnonzero(diff_pm_fa < 0)[-1]
    #Debug
    valid_x2 = np.flatnonzero(diff_pm_fa < 0)
    if len(valid_x2) == 0:
        print("Warning: No valid x2 found in compute_eer. Returning default EER value.")
        return 1.0, None
    x2 = valid_x2[-1]

    # a = (fnr[x1] - fpr[x1]) / (fpr[x2] - fpr[x1] - (fnr[x2] - fnr[x1]))
    #Debug
    denominator = (fpr[x2] - fpr[x1]) - (fnr[x2] - fnr[x1])
    if denominator == 0:
        print("Warning: Division by zero in compute_eer. Returning default EER value.")
        return 1.0, None

    # Compute interpolation factor
    a = (fnr[x1] - fpr[x1]) / denominator
