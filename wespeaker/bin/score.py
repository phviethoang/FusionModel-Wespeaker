...
def trials_cosine_score(eval_scp_path='',
    ...
    for trial in trials:
        ...
        with open(trial, 'r') as trial_r, open(store_path, 'w') as w_f:
            ...
            skipped_count = 0  # Đếm số dòng bị bỏ qua
            valid_count = 0  # Đếm số dòng hợp lệ

            for line in tqdm(lines, desc=f'Scoring trial {os.path.basename(trial)}'):
                segs = line.strip().split()

                # Kiểm tra xem cả hai file có trong embeddings không
                if segs[0] not in emb_dict or segs[1] not in emb_dict:
                    print(f"⚠ Warning: Skipping missing embedding for {segs[0]} or {segs[1]}")
                    skipped_count += 1
                ...
                if len(segs) == 3:  # enroll_name test_name target/nontarget
                    w_f.write(f'{segs[0]} {segs[1]} {cos_score:.5f} {segs[2]}\n')
                else:  # enroll_name test_name
                    w_f.write(f'{segs[0]} {segs[1]} {cos_score:.5f}\n')

                valid_count += 1  # Đếm số dòng hợp lệ

            print(f" Finished scoring {os.path.basename(trial)}. Skipped: {skipped_count}, Processed: {valid_count}")
