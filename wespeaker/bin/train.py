...
def train(config='conf/config.yaml', **kwargs):
    ...
    # Tính số chiều đầu vào sau khi concatenate
    fbank_dim = configs['dataset_args'].get('feat_dim', 80)  # Mặc định fbank có 80 chiều
    if use_fusion:
        input_dim = fbank_dim + frontend_output_dim
    else:
        input_dim = fbank_dim if frontend_type == "fbank" else frontend_output_dim
    # Khởi tạo speaker model
    model = get_speaker_model(configs['model'])(**configs['model_args'])
    
    # Thêm frontend nếu có
    if frontend_type != "fbank":
        model.add_module("frontend", frontend)
    # Thêm fusion module nếu cần
    if use_fusion:
        fusion_module = CrossAttentionFusion(
            fbank_dim=fbank_dim,
            wavlm_dim=frontend_output_dim,
            d_model=configs['model_args']['feat_dim'],  # Ví dụ: 80
            num_heads=1,  # Số head trong multi-head attention
            dropout=0.0
        )
    ...
    if rank == 0:
        ...
        logger.info('trainable speaker_model size: {}'.format(train_params))
    ...
    # if frontend_type != "fbank":
    #     model.add_module("frontend", frontend)
    # # Thêm fusion projection layer nếu cần
    # if use_fusion:
    #     # Projection layer để chiếu từ (fbank_dim + frontend_output_dim) xuống feat_dim
    ...
    for idx, (name, param) in enumerate(ddp_model.named_parameters()):
